...
const CouchSystem = {
    addCushion(player, block, woodType, isStool = false) {
        ...
        const fillCommands = directions.map(dir =>
            `${targetType}["minecraft:cardinal_direction"="${dir}"]`
        ).join(' ');
        
        block.dimension.runCommand(
            `fill ${x} ${y} ${z} ${x} ${y} ${z} ${fillCommands} replace ${baseType}`
        );
    },
    ...
    removeCushion(player, block, woodType, isStool = false) {
        ...
        const newPermutation = BlockPermutation.resolve(baseType);
        block.setPermutation(newPermutation);
        
        try {
            const face = block.permutation.getState("minecraft:cardinal_direction");
            if (face !== undefined) {
                block.setPermutation(block.permutation.withState("minecraft:cardinal_direction", face));
            }
        } catch (error) {
...
const CurtainSystem = {
    ...
    isCurtainBlock(block) {
        ...
        return block.typeId.startsWith('ff:white_curtain_') &&
                WOOD_TYPES.some(type => block.typeId.endsWith(type));
    },
    ...
    ffh.blockComponentRegistry.registerCustomComponent('ff:fridge_dual', {
        ...
        onPlace(e) {
            ...
            if (blockAbove && blockAbove.typeId === 'minecraft:air') {
                ...
                block.dimension.spawnEntity("ff:fridge_inventory", {
                    x: x + 0.5,
                    y: y + 1.5,
                    z: z + 0.5
                });
                block.dimension.spawnEntity("ff:fridge_inventory_freezer", {
                    x: x + 0.5,
                    y: y + 0.5,
                    z: z + 0.5
                });
                ...
                if (inv?.container) {
                    for (let slot = 0; slot < 5; slot++) {
                        ...
                        if (item && freezeMap[item.typeId]) {
                            ...
                            inv.container.setItem(slot, frozenItem);
                        }
                    }
                }
            }
    ...
    ffh.blockComponentRegistry.registerCustomComponent('ff:outlet_function', {
        ...
        onTick: e => {
            ...
            if (currentState === 1) {
                ...
                block.dimension.spawnEntity("minecraft:ender_crystal", { x: x + 0.5, y: y + 0.5, z: z + 0.5 });
        ...
        onPlayerInteract: e => {
            ...
            if (
                ...
                if (player.getGameMode() !== GameMode.creative) {
                    ...
                    const added = inventory.addItem(waterBucket);
                    if (added === undefined || added) {
                        ...
                        player.sendMessage('§cNo tienes espacio para la cubeta de agua.');
                    }
                } else {
                    equipment.setEquipment('Mainhand', new ItemStack('minecraft:water_bucket', 1));
                    Utils.playSoundForPlayer(player, 'bucket.fill_water');
                }
                return;
            }

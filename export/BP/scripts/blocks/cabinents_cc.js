...
world.beforeEvents.worldInitialize.subscribe(ffh => {
  ...
  const frontZones2 = new FaceSelectionPlains(
    { origin: [2, 4], size: [12, 5] },
    { origin: [2, 4], size: [12, 5] },
    { origin: [2, 4], size: [12, 5] }
  );
  ...
  ffh.blockComponentRegistry.registerCustomComponent(
    "ff:container_function_table",
    {
      ...
      onPlayerInteract: e => {
        ...
        const zone = frontZones2.getSelected({ face, faceLocation: rel });
        if (zone === undefined) return;
...
world.afterEvents.playerInteractWithBlock.subscribe(e => {
  ...
  const stoneMap = {
    ...
    "minecraft:pale_oak_planks": { counter: 11 },

...
world.afterEvents.playerInteractWithBlock.subscribe(async e => {
  ...
  if (id === "ff:modern_switch" && !isWrench) {
    ...
    const st = gen && genState.get(gen.id);
    if (!gConn) {
      player.sendMessage("§c The block is not linked to any generator");
      return;
    }
    if (!st || !st.enabled || st.energy <= 0) {
  ...
  if (outletTargets.includes(id) && !isWrench) {
    ...
    if (!outletConn) {
      player.sendMessage("§7 The block is not linked to any outlet");
      return;
    }
  ...
  if (sourceId === GENERATOR_ID) {
    ...
    if (genConns.length >= MAX_GEN_CONNS) {
      player.sendMessage("§c The generator is full!");
      clearSelection(player);
      return;
    }
  ...
  if (sourceId === "ff:outlet") {
    ...
    if (outletConns.length > 0) {
      ...
      if (segments.length >= OUTLET_MAX_CHAINS) {
        player.sendMessage("§c The outlet is fully, link removed");
        clearSelection(player);
        return;
      }
    }
  ...
  for (const typeId of blockTypeChanges) {
    ...
    if (outletConns.length > 0) {
      ...
      if (segments.length > OUTLET_MAX_CHAINS) {
        ...
        saveConnections(newConns);
        for(const p of world.getPlayers()) {
          p.sendMessage("§c The outlet is fully, link removed");
        }
      }
    }
  }
...
// --- FUNCIÓN HELPER PARA LIMPIAR ENTIDADES DE LÁMPARAS ---
...
function cleanLampEntitiesInChain(block) {
  ...
  // Encontrar todos los bloques en la cadena vertical
  while (toClean.length > 0) {
    ...
    // Limpiar entidad del bloque actual
    cleanLampEntity(current);
    
    // Agregar bloques adyacentes del mismo tipo

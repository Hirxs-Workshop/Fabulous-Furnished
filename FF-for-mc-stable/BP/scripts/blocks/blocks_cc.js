      ...
      registerRemoveCouch(
        ...
        block => {
          ...
          return `fill ${x} ${y} ${z} ${x} ${y} ${z}` +
                 ` ff:wooden_${type}_stool["ff:block_rotation"=${rotation},"minecraft:block_face"="${face}"]` +
                 ` replace ff:wooden_${type}_stool_with_wool["ff:block_rotation"=${rotation},"minecraft:block_face"="${face}"]`;
        }
      );
    }
  });
...
// --- SISTEMA DE CONEXIÓN DE CORTINAS (ff:white_curtain) COMO COMPONENTE CUSTOM ---

function updateCurtainConnectionsZYCardinal(block) {
    if (!block || block.typeId !== "ff:white_curtain") return;
    const dim = block.dimension;
    ...
    // Limpiar estados de los vecinos en un área 3x3x3 (X, Y, Z)
    for (let dx = -1; dx <= 1; dx++) {
        for (let dy = -1; dy <= 1; dy++) {
            for (let dz = -1; dz <= 1; dz++) {
                const b = dim.getBlock({x: x + dx, y: y + dy, z: z + dz});
                if (b && b.typeId === "ff:white_curtain" && b.permutation.getState("minecraft:cardinal_direction") === cardinal) {
                    b.setPermutation(b.permutation.withState("ff:connection", 0));
                }
            }
        }
    }
    ...
    for (const [dy, dz] of matrices) {
        const by = y + dy;
        const bz = z + dz;
        const blocks = [
            dim.getBlock({x, y: by,   z: bz    }), // inf izq
            dim.getBlock({x, y: by,   z: bz+1  }), // inf der
            dim.getBlock({x, y: by+1, z: bz    }), // sup izq
            dim.getBlock({x, y: by+1, z: bz+1  })  // sup der
        ];
        if (blocks.every(b => b && b.typeId === "ff:white_curtain" && b.permutation.getState("minecraft:cardinal_direction") === cardinal)) {
            blocks[0].setPermutation(blocks[0].permutation.withState("ff:connection", 1));
            blocks[1].setPermutation(blocks[1].permutation.withState("ff:connection", 2));
            blocks[2].setPermutation(blocks[2].permutation.withState("ff:connection", 3));
            blocks[3].setPermutation(blocks[3].permutation.withState("ff:connection", 4));
            return;
        }
    }

    // 2. Fila de 3 (horizontal, X)
    const left = dim.getBlock({x: x-1, y: y, z: z});
    const right = dim.getBlock({x: x+1, y: y, z: z});
    if (
        left && left.typeId === "ff:white_curtain" && left.permutation.getState("minecraft:cardinal_direction") === cardinal &&
        right && right.typeId === "ff:white_curtain" && right.permutation.getState("minecraft:cardinal_direction") === cardinal
    ) {
        left.setPermutation(left.permutation.withState("ff:connection", 1));
        block.setPermutation(block.permutation.withState("ff:connection", 5));
        right.setPermutation(right.permutation.withState("ff:connection", 2));
        return;
    }

    // 3. Columna de 3 (vertical, Y)
    const up = dim.getBlock({x: x, y: y+1, z: z});
    const down = dim.getBlock({x: x, y: y-1, z: z});
    if (
        up && up.typeId === "ff:white_curtain" && up.permutation.getState("minecraft:cardinal_direction") === cardinal &&
        down && down.typeId === "ff:white_curtain" && down.permutation.getState("minecraft:cardinal_direction") === cardinal
    ) {
        up.setPermutation(up.permutation.withState("ff:connection", 3));
        block.setPermutation(block.permutation.withState("ff:connection", 6));
        down.setPermutation(down.permutation.withState("ff:connection", 4));
        return;
    }
    // Si no es parte de ninguna, queda en 0
}

world.beforeEvents.worldInitialize.subscribe(ffh => {
    ffh.blockComponentRegistry.registerCustomComponent('ff:curtain_connection', {
        onTick: e => {
            const { block } = e;
            updateCurtainConnectionsZYCardinal(block);
            // También actualiza vecinos en Y y Z (no X) para robustez ante destrucción
            for (const dy of [-1, 0, 1]) {
                for (const dz of [-1, 0, 1]) {
                    if (dy !== 0 || dz !== 0) {
                        const neighbor = block.dimension.getBlock({x: block.location.x, y: block.location.y + dy, z: block.location.z + dz});
                        if (neighbor && neighbor.typeId === "ff:white_curtain") {
                            updateCurtainConnectionsZYCardinal(neighbor);
                        }
                    }
                }
            }
...
// --- FIN SISTEMA DE CONEXIÓN DE CORTINAS COMO COMPONENTE CUSTOM ---

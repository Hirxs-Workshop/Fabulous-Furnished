    ...
    initEvent.blockComponentRegistry.registerCustomComponent('sd:leaves_wall_decors_place', {
      onPlace: e => {
        ...
        if (block.permutation.getState("sd:rotation") === 0 && block.permutation.getState("minecraft:block_face") === 'south') {
          block.setPermutation(small);
          block.dimension.runCommand(`summon sd:leaves_wall_decor_entity ${x} ${y} ${z} 0 0`);
          return;
        }
        if (block.permutation.getState("sd:rotation") === 0 && block.permutation.getState("minecraft:block_face") === 'north') {
          block.setPermutation(small);
          block.dimension.runCommand(`summon sd:leaves_wall_decor_entity ${x} ${y} ${z} 180 0`);
          return;
        }
        if (block.permutation.getState("sd:rotation") === 0 && block.permutation.getState("minecraft:block_face") === 'west') {
          block.setPermutation(small);
          block.dimension.runCommand(`summon sd:leaves_wall_decor_entity ${x} ${y} ${z} 90 0`);
          return;
        }
        ...
        if (block.permutation.getState("sd:rotation") === 0) {
          ...
          if (block.permutation.getState("sd:rotation") === 1) {
            block.dimension.runCommand(`summon sd:leaves_wall_decor_entity ${x} ${y} ${z} 200 0`);
            block.setPermutation(small);
            return;
          }
          if (block.permutation.getState("sd:rotation") === 2) {
            block.dimension.runCommand(`summon sd:leaves_wall_decor_entity ${x} ${y} ${z} 225 0`);
            block.setPermutation(small);
            return;
          }
          if (block.permutation.getState("sd:rotation") === 3) {
            block.dimension.runCommand(`summon sd:leaves_wall_decor_entity ${x} ${y} ${z} 250 0`);
            block.setPermutation(small);
            return;
          }
          if (block.permutation.getState("sd:rotation") === 4) {
            block.dimension.runCommand(`summon sd:leaves_wall_decor_entity ${x} ${y} ${z} 270 0`);
            block.setPermutation(small);
            return;
          }
          if (block.permutation.getState("sd:rotation") === 5) {
            block.dimension.runCommand(`summon sd:leaves_wall_decor_entity ${x} ${y} ${z} 290 0`);
            block.setPermutation(small);
            return;
          }
          if (block.permutation.getState("sd:rotation") === 6) {
            block.dimension.runCommand(`summon sd:leaves_wall_decor_entity ${x} ${y} ${z} 315 0`);
            block.setPermutation(small);
            return;
          }
          if (block.permutation.getState("sd:rotation") === 7) {
            block.dimension.runCommand(`summon sd:leaves_wall_decor_entity ${x} ${y} ${z} 335 0`);
            block.setPermutation(small);
            return;
          }
          ...
          if (block.permutation.getState("sd:rotation") === 9) {
            block.dimension.runCommand(`summon sd:leaves_wall_decor_entity ${x} ${y} ${z} 25 0`);
            block.setPermutation(small);
            return;
          }
          if (block.permutation.getState("sd:rotation") === 10) {
            block.dimension.runCommand(`summon sd:leaves_wall_decor_entity ${x} ${y} ${z} 45 0`);
            block.setPermutation(small);
            return;
          }
          if (block.permutation.getState("sd:rotation") === 11) {
            block.dimension.runCommand(`summon sd:leaves_wall_decor_entity ${x} ${y} ${z} 70 0`);
            block.setPermutation(small);
            return;
          }
          ...
          if (block.permutation.getState("sd:rotation") === 13) {
            block.dimension.runCommand(`summon sd:leaves_wall_decor_entity ${x} ${y} ${z} 115 0`);
            block.setPermutation(small);
            return;
          }
          if (block.permutation.getState("sd:rotation") === 14) {
            block.dimension.runCommand(`summon sd:leaves_wall_decor_entity ${x} ${y} ${z} 135 0`);
            block.setPermutation(small);
            return;
          }
          if (block.permutation.getState("sd:rotation") === 15) {
            block.dimension.runCommand(`summon sd:leaves_wall_decor_entity ${x} ${y} ${z} 160 0`);
            block.setPermutation(small);
            return;
          }
      },
  ...
  system.beforeEvents.startup.subscribe(initEvent => {
    initEvent.blockComponentRegistry.registerCustomComponent('sd:wall_decors_place_lights', {
      onPlace: e => {
        ...
        if (block.permutation.getState("sd:rotation") === 0 && block.permutation.getState("minecraft:block_face") === 'south') {
          block.setPermutation(small);
          block.dimension.runCommand(`summon sd:christmas_lights_entity ${x} ${y} ${z} 0 0`);
          return;
        }
        if (block.permutation.getState("sd:rotation") === 0 && block.permutation.getState("minecraft:block_face") === 'north') {
          block.setPermutation(small);
          block.dimension.runCommand(`summon sd:christmas_lights_entity ${x} ${y} ${z} 180 0`);
          return;
        }
        if (block.permutation.getState("sd:rotation") === 0 && block.permutation.getState("minecraft:block_face") === 'west') {
          block.setPermutation(small);
          block.dimension.runCommand(`summon sd:christmas_lights_entity ${x} ${y} ${z} 90 0`);
          return;
        }
        if (block.permutation.getState("sd:rotation") === 0 && block.permutation.getState("minecraft:block_face") === 'east') {
          block.setPermutation(small);
        ...
        if (block.permutation.getState("sd:rotation") === 0) {
          ...
          if (block.permutation.getState("sd:rotation") === 1) {
            block.dimension.runCommand(`summon sd:christmas_lights_entity ${x} ${y} ${z} 200 0`);
            block.setPermutation(small);
            return;
          }
          if (block.permutation.getState("sd:rotation") === 2) {
            block.dimension.runCommand(`summon sd:christmas_lights_entity ${x} ${y} ${z} 225 0`);
            block.setPermutation(small);
            return;
          }
          if (block.permutation.getState("sd:rotation") === 3) {
            block.dimension.runCommand(`summon sd:christmas_lights_entity ${x} ${y} ${z} 250 0`);
            block.setPermutation(small);
            return;
          }
          if (block.permutation.getState("sd:rotation") === 4) {
            block.dimension.runCommand(`summon sd:christmas_lights_entity ${x} ${y} ${z} 270 0`);
            block.setPermutation(small);
            return;
          }
          if (block.permutation.getState("sd:rotation") === 5) {
            block.dimension.runCommand(`summon sd:christmas_lights_entity ${x} ${y} ${z} 290 0`);
            block.setPermutation(small);
            return;
          }
          if (block.permutation.getState("sd:rotation") === 6) {
            block.dimension.runCommand(`summon sd:christmas_lights_entity ${x} ${y} ${z} 315 0`);
            block.setPermutation(small);
            return;
          }
          if (block.permutation.getState("sd:rotation") === 7) {
            block.dimension.runCommand(`summon sd:christmas_lights_entity ${x} ${y} ${z} 335 0`);
            block.setPermutation(small);
            return;
          }
          if (block.permutation.getState("sd:rotation") === 8) {
          ...
          if (block.permutation.getState("sd:rotation") === 9) {
            block.dimension.runCommand(`summon sd:christmas_lights_entity ${x} ${y} ${z} 25 0`);
            block.setPermutation(small);
            return;
          }
          if (block.permutation.getState("sd:rotation") === 10) {
            block.dimension.runCommand(`summon sd:christmas_lights_entity ${x} ${y} ${z} 45 0`);
            block.setPermutation(small);
            return;
          }
          if (block.permutation.getState("sd:rotation") === 11) {
            block.dimension.runCommand(`summon sd:christmas_lights_entity ${x} ${y} ${z} 70 0`);
            block.setPermutation(small);
            return;
          }
          if (block.permutation.getState("sd:rotation") === 12) {
          ...
          if (block.permutation.getState("sd:rotation") === 13) {
            block.dimension.runCommand(`summon sd:christmas_lights_entity ${x} ${y} ${z} 115 0`);
            block.setPermutation(small);
            return;
          }
          if (block.permutation.getState("sd:rotation") === 14) {
            block.dimension.runCommand(`summon sd:christmas_lights_entity ${x} ${y} ${z} 135 0`);
            block.setPermutation(small);
            return;
          }
          if (block.permutation.getState("sd:rotation") === 15) {
            block.dimension.runCommand(`summon sd:christmas_lights_entity ${x} ${y} ${z} 160 0`);
            block.setPermutation(small);
            return;
          }
      },
